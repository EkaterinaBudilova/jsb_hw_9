...
requestColors.send();
requestColors.addEventListener('load', () => {
  processRequestResponse(requestColors, showColors);
...
requestSizes.send();
requestSizes.addEventListener('load', () => {
  processRequestResponse(requestSizes, showSizes);
...
document.getElementById('AddToCartForm').addEventListener('submit', (event) => {
  ...
  xhrCart.addEventListener('load', () => {
    processRequestResponse(xhrCart, showCart);
  });
  ...
  Array.from(document.querySelectorAll('.quick-cart-product-remove')).forEach(removeButton => {
    removeButton.addEventListener('click', (event) => {
      ...
      xhrRemove.addEventListener('load', () => {
        processRequestResponse(xhrRemove, showCart);
      });
...
function processRequestResponse(request, callback) {
  try {
